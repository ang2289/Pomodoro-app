...
const SUPABASE_MAINTENANCE_PREFIXES = [
  '/admin',
  '/login',
  '/register',
  '/reset',
  ...
  '/tools/commercial-image',
  '/services/product-image-upgrade',
...
const IMAGE_BUNDLE_PUBLIC_PAYMENT_PATHS = new Set([
  '/payment/bank-transfer',
  '/payment/report',
])

const LOCAL_ADMIN_IMAGE_PATHS = new Set([
  '/admin/images',
  '/admin/images/list',
])

function isLocalAdminImagePath(pathname: string) {
  if (!LOCAL_ADMIN_IMAGE_PATHS.has(pathname) || typeof window === 'undefined') return false
  return window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1'
}

function isSupabaseMaintenancePath(pathname: string, search = '') {
  if (isLocalAdminImagePath(pathname)) return false

  if (
    new URLSearchParams(search).get('product') === 'image-bundle-full' &&
    IMAGE_BUNDLE_PUBLIC_PAYMENT_PATHS.has(pathname)
  ) {
    return false
  }

  return SUPABASE_MAINTENANCE_PREFIXES.some((prefix) =>
...
function App() {
  ...
  const isSupabaseMaintenancePage =
    !featureFlags.supabaseFeaturesEnabled && isSupabaseMaintenancePath(location.pathname, location.search)

  if (isSupabaseMaintenancePage) {
    return <SupabaseMaintenancePage admin={location.pathname.startsWith('/admin')} />
  }
