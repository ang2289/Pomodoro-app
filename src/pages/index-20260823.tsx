...
const RXV_ADMIN_EMAILS = ["ang2289@gmail.com", "ang2289@yahoo.com.tw"];

// 團購功能暫停：保留程式碼與資料，首頁不顯示任何亞尼克／團購入口。
...
type CustomerServiceStatus = {
  ...
  storefrontTrialRequest?: { id: string; status: string; createdAt?: string | null; reviewedAt?: string | null } | null;
...
const HomePage: React.FC = () => {
  ...
  const [pendingImageBundlePaymentCount, setPendingImageBundlePaymentCount] = useState(0);
  const [pendingImageBundleDeliveryCount, setPendingImageBundleDeliveryCount] = useState(0);
  ...
  useEffect(() => {
    if (!loggedIn || !canShowAdminReminders) {
      ...
      setPendingImageBundlePaymentCount(0);
      setPendingImageBundleDeliveryCount(0);
    ...
    const loadPendingAdminItems = async () => {
      try {
        const [bankResponse, businessCardResponse, imageBundleResponse] = await Promise.all([
          ...
          fetch("/api/main?action=admin-get-digital-product-bundle-summary", {
            method: "GET",
            headers: { Authorization: `Bearer ${token}` },
          }),
        ]);
        ...
        const imageBundleData = await imageBundleResponse.json().catch(() => ({}));
        ...
        if (!cancelled) {
          ...
          setPendingImageBundlePaymentCount(Number(imageBundleData?.pendingPaymentCount || 0));
          setPendingImageBundleDeliveryCount(Number(imageBundleData?.pendingDeliveryCount || 0));
  ...
  const hasPendingImageBundlePayments = pendingImageBundlePaymentCount > 0;
  const hasPendingImageBundleDeliveries = pendingImageBundleDeliveryCount > 0;
    ...
    const loadCustomerServiceStatus = async () => {
      try {
        ...
        if (!cancelled) setCustomerServiceStatus({
          ...
          storefrontTrialRequest: data?.storefrontTrialRequest || null,
  ...
  const storefrontTrialStatus = String(customerServiceStatus?.storefrontTrialRequest?.status || "").toLowerCase();
  const hasPendingStorefrontTrialRequest = !hasActiveStorefrontAccess && storefrontTrialStatus === "pending";
  ...
  const openStorefrontSettings = (source: "business-card" | "product-image") => {
    ...
    if (hasPendingStorefrontTrialRequest) {
      navigate("/tools/product-showcase-page#trial");
      return;
    }
  ...
  const homeShareTitle = "RxV 圖片素材與 LINE 貼圖工具";
  const homeShareText =
    "部分圖片免費下載，也可取得 1,583+ 張完整素材庫 NT$399；另提供 LINE 貼圖整理、圖片尺寸與創作工具。";
  ...
  const toolCards: ToolCard[] = [
    ...
    },
    {
      id: "free-resources",
      titleKey: "免費資源中心",
      descriptionKey: "LINE 貼圖咒語、AI 生圖提示詞、文案模板與免費圖片資源下載。",
      icon: "🎁",
      href: "/free",
      categoryKey: "home_category_design",
      ringColor: "ring-pink-100",
    ...
    ...(isChantWishEnabled
      ...
      : []),
    // 團購目前暫停；保留設定，日後只要將 SHOW_GROUP_BUY 改為 true 即可恢復入口。
    ...(SHOW_GROUP_BUY
      ? [{
          id: "group-buy-public",
          titleKey: "亞尼克第一團｜商品登記",
          descriptionKey: "查看可登記商品與目前累計數量；達成團門檻後才通知付款。",
          icon: "🧺",
          href: "/group-buy/yannick-first-group-buy",
          categoryKey: "home_category_productivity",
      ...
      : []),
    ...(SHOW_GROUP_BUY && isAdminEmail
      ? [{
          id: "group-buy-admin",
          titleKey: "團購管理",
          descriptionKey: "指定管理者使用：建立活動、核對付款、管理訂貨與出貨狀態。",
          icon: "🔐",
  ...
  // 首頁上方已主打的卡片，不要在下方「所有工具」再次重複出現。
  const HOME_FEATURED_CARD_IDS = new Set([
    "images",
    "sticker-prompt",
    "line-sticker",
    "image-prompt",
  ...
  const homeToolGroups = [
    ...
    .map((group) => ({ ...group, items: homeQuickToolButtons.filter(group.match).filter((card) => !HOME_FEATURED_CARD_IDS.has(card.id)) }))
    ...
    return (
      ...
      >
        ...
        <p className="mt-2 line-clamp-3 text-sm leading-6 text-slate-600">
          {card.id === "images"
            ? "部分圖片免費下載；完整版 1,583+ 張素材庫 NT$399，一次取得。"
            : t(card.descriptionKey)}
        </p>
        <span className="mt-auto pt-4 text-sm font-black text-sky-700">開啟工具 →</span>
      </Link>
    );
  };
  ...
  const featuredTools = [
    ...
    },
    {
      ...
      icon: "🎁",
      titleKey: "免費資源中心",
      descKey: "LINE 貼圖咒語、AI 生圖提示詞、文案模板與免費圖片資源下載。",
      href: "/free",
    },
    {
      id: "image-resize",
    ...
    <>
      ...
      <div className="min-h-screen bg-gradient-to-b from-sky-50 via-white to-indigo-50">
        ...
        <div className="mx-auto w-full max-w-6xl px-4 pt-5 sm:px-6 lg:px-8">
          <div className="overflow-hidden rounded-[1.5rem] border border-emerald-100 bg-white shadow-sm">
              ...
              {loggedIn ? (
                <div className="mt-5 flex flex-wrap items-center gap-3">
                  ...
                  {SHOW_GROUP_BUY && isAdminEmail && (
                    <Link
                      to="/admin/group-buy"
                      className="inline-flex min-h-[48px] w-fit items-center justify-center rounded-xl bg-orange-600 px-5 py-3 text-sm font-black !text-white shadow-md transition hover:-translate-y-0.5 hover:bg-orange-700 hover:shadow-lg"
                      style={{ color: "#ffffff", WebkitTextFillColor: "#ffffff" }}
                    >
                      🧺 團購管理
                    </Link>
                  ...
                  <section className="w-full rounded-2xl border border-sky-100 bg-sky-50/70 p-4 shadow-sm">
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                      <div className="min-w-0">
                        ...
                        <p className={`mt-2 text-sm font-bold ${
                          hasActiveStorefrontAccess
                            ? "text-emerald-700"
                            : hasPendingStorefrontTrialRequest
                              ? "text-amber-700"
                              : "text-slate-500"
                        }`}>
                          {hasActiveStorefrontAccess
                      ...
                      {hasActiveStorefrontAccess ? (
                        ...
                        <span className="inline-flex w-fit shrink-0 items-center rounded-full bg-amber-100 px-3 py-1.5 text-xs font-black text-amber-800">
                          試用申請審核中
                        </span>
                      ) : null}
                    </div>
                    ...
                    {hasPendingStorefrontTrialRequest ? (
                      <div className="mt-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-bold leading-relaxed text-amber-900">
                        商品展示頁 7 天試用申請開通中，審核通過後首頁會改成「已開通」，即可進入設定頁建立商品展示內容。
                      </div>
                    ) : null}
                  </section>
                  ...
                  {canShowAdminReminders ? (
                    <div className="flex w-full flex-wrap gap-2">
                      ...
                      <Link
                        to="/admin/payments?section=image-bundle"
                        className={`inline-flex min-h-[48px] items-center justify-center rounded-xl px-5 py-3 text-sm font-black !text-white shadow-md transition hover:-translate-y-0.5 hover:shadow-lg ${hasPendingImageBundlePayments ? "bg-rose-600 hover:bg-rose-700" : "bg-slate-600 hover:bg-slate-700"}`}
                        style={{ color: "#ffffff", WebkitTextFillColor: "#ffffff" }}
                      >
                        {`待核對圖片素材庫 NT$399 ${pendingImageBundlePaymentCount} 筆`}
                      </Link>
                      <Link
                        to="/admin/payments?section=image-bundle"
                        className={`inline-flex min-h-[48px] items-center justify-center rounded-xl px-5 py-3 text-sm font-black !text-white shadow-md transition hover:-translate-y-0.5 hover:shadow-lg ${hasPendingImageBundleDeliveries ? "bg-rose-600 hover:bg-rose-700" : "bg-emerald-600 hover:bg-emerald-700"}`}
                        style={{ color: "#ffffff", WebkitTextFillColor: "#ffffff" }}
                      >
                        {`圖片素材庫待交付 ${pendingImageBundleDeliveryCount} 筆`}
                      </Link>
                    </div>
                  ) : null}
                </div>
            ...
            <div className="border-t border-emerald-100 bg-gradient-to-r from-cyan-50 via-white to-emerald-50 p-5 sm:p-6">
              ...
              <div className={`mt-5 grid w-full gap-3 ${loggedIn ? 'sm:grid-cols-2 xl:grid-cols-4' : 'sm:grid-cols-2 lg:grid-cols-3'}`}>
                <Link
                  to="/tools/product-showcase-page#trial"
                  className="inline-flex min-h-[76px] items-center justify-center rounded-xl bg-gradient-to-r from-emerald-600 to-teal-500 px-4 py-3 text-center text-sm font-black leading-snug !text-white shadow-md transition hover:-translate-y-0.5 hover:from-emerald-700 hover:to-teal-600 hover:shadow-lg"
                  style={{ color: "#ffffff", WebkitTextFillColor: "#ffffff", wordBreak: "keep-all" }}
                >
                  <span className="block">
                    免費試用 7 天
                    <br />
                    <span className="whitespace-nowrap text-xs sm:text-sm">先建立 1 個商品頁</span>
                  </span>
                </Link>
                <Link
                  to="/tools/product-showcase-page"
                  className="inline-flex min-h-[76px] items-center justify-center rounded-xl bg-gradient-to-r from-cyan-600 to-sky-500 px-4 py-3 text-center text-sm font-black leading-snug !text-white shadow-md transition hover:-translate-y-0.5 hover:from-cyan-700 hover:to-sky-600 hover:shadow-lg"
                  style={{ color: "#ffffff", WebkitTextFillColor: "#ffffff", wordBreak: "keep-all" }}
                >
                  <span className="block">
                    正式版 NT$199
                    <br />
                    <span className="whitespace-nowrap">使用 3 個月</span>
                  </span>
                </Link>
                <Link
                  to="/shop/rxv"
                  className="inline-flex min-h-[76px] items-center justify-center rounded-xl border border-emerald-200 bg-white px-4 py-3 text-center text-sm font-black leading-snug text-emerald-700 shadow-sm transition hover:-translate-y-0.5 hover:border-emerald-300 hover:bg-emerald-50 hover:shadow-md"
                  style={{ wordBreak: "keep-all" }}
                >
                  <span className="block">看看商品頁示範</span>
                </Link>
                {loggedIn ? (
                  <button
                    type="button"
                    onClick={() => openStorefrontSettings("product-image")}
                    className="inline-flex min-h-[76px] items-center justify-center rounded-xl border border-sky-200 bg-white px-4 py-3 text-center text-sm font-black leading-snug text-sky-700 shadow-sm transition hover:-translate-y-0.5 hover:border-sky-300 hover:bg-sky-50 hover:shadow-md"
                    style={{ wordBreak: "keep-all" }}
                  >
                    <span className="block">
                      設定我的
        ...
        <div className="mx-auto w-full max-w-6xl px-4 py-8 sm:py-10 lg:py-12 flex flex-col">
          {/* Hero 區：改為與工具專區一致的卡片版面 */}
          <section className="mb-8 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm order-2 sm:p-6">
            <div className="mb-5 flex flex-wrap items-end justify-between gap-3">
              <div>
                ...
                <h1 className="mt-3 text-2xl font-black tracking-tight text-slate-900 sm:text-3xl">
                  圖片素材與 LINE 貼圖工具，一站直接使用
                </h1>
                <p className="mt-2 max-w-3xl text-sm leading-relaxed text-slate-600 sm:text-base">
                  免費圖片、完整版素材庫、LINE 貼圖整理、生圖提示詞與圖片工具，依需求直接選擇。
                </p>
              </div>
              <Link
                to="/tools"
                className="inline-flex min-h-[42px] items-center rounded-full bg-sky-600 px-4 py-2 text-sm font-bold !text-white shadow-sm transition hover:bg-sky-700 hover:shadow-md"
                style={{ color: "#ffffff", WebkitTextFillColor: "#ffffff" }}
              >
                查看全部工具
              </Link>
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              <Link
                to="/images"
                className={`${POPULAR_TOOL_CARD_CLASS} border-l-4 border-l-emerald-500`}
              >
                <div className="flex items-start justify-between gap-3">
                  <span className="text-2xl" aria-hidden>🖼️</span>
                  <span className="rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-black text-emerald-700">免費＋完整版</span>
                </div>
                <h2 className="mt-3 text-lg font-black text-slate-900">圖片素材庫</h2>
                <p className="mt-2 text-sm leading-relaxed text-slate-600">
                  部分圖片可免費下載，也可取得 1,583+ 張完整素材庫。
                </p>
              </Link>

              <Link
                to="/tools/sticker-prompt"
                className={`${POPULAR_TOOL_CARD_CLASS} border-l-4 border-l-violet-500`}
              >
                <div className="flex items-start justify-between gap-3">
                  <span className="text-2xl" aria-hidden>💬</span>
                  <span className="rounded-full bg-violet-100 px-2.5 py-1 text-xs font-black text-violet-700">貼圖</span>
                </div>
                <h2 className="mt-3 text-lg font-black text-slate-900">LINE 貼圖提示詞</h2>
                <p className="mt-2 text-sm leading-relaxed text-slate-600">
                  快速整理角色、動作、文字與系列貼圖提示詞。
                </p>
              </Link>

              <Link
                to="/tools/line-sticker"
                className={`${POPULAR_TOOL_CARD_CLASS} border-l-4 border-l-cyan-500`}
              >
                <div className="flex items-start justify-between gap-3">
                  <span className="text-2xl" aria-hidden>🧩</span>
                  <span className="rounded-full bg-cyan-100 px-2.5 py-1 text-xs font-black text-cyan-700">ZIP</span>
                </div>
                <h2 className="mt-3 text-lg font-black text-slate-900">LINE 貼圖分割打包</h2>
                <p className="mt-2 text-sm leading-relaxed text-slate-600">
                  4x4／5x4 大圖分割、整理尺寸並打包下載。
                </p>
              </Link>

              <Link
                to="/tools/image-prompt"
                className={`${POPULAR_TOOL_CARD_CLASS} border-l-4 border-l-fuchsia-500`}
              >
                <div className="flex items-start justify-between gap-3">
                  <span className="text-2xl" aria-hidden>✨</span>
                  <span className="rounded-full bg-fuchsia-100 px-2.5 py-1 text-xs font-black text-fuchsia-700">AI</span>
                </div>
                <h2 className="mt-3 text-lg font-black text-slate-900">AI 生圖提示詞</h2>
                <p className="mt-2 text-sm leading-relaxed text-slate-600">
                  商品圖、社群圖、角色圖與封面圖提示詞快速產生。
                </p>
              </Link>

              <Link
                to="/tools/image-resize"
                className={`${POPULAR_TOOL_CARD_CLASS} border-l-4 border-l-sky-500`}
              >
                <div className="flex items-start justify-between gap-3">
                  <span className="text-2xl" aria-hidden>📐</span>
                  <span className="rounded-full bg-sky-100 px-2.5 py-1 text-xs font-black text-sky-700">圖片</span>
                </div>
                <h2 className="mt-3 text-lg font-black text-slate-900">圖片尺寸轉換</h2>
                <p className="mt-2 text-sm leading-relaxed text-slate-600">
                  快速調整圖片尺寸，適合社群、商品頁與貼圖製作。
                </p>
              </Link>

              <Link
                to="/free"
                className={`${POPULAR_TOOL_CARD_CLASS} border-l-4 border-l-pink-500`}
              >
                <div className="flex items-start justify-between gap-3">
                  <span className="text-2xl" aria-hidden>🎁</span>
                  <span className="rounded-full bg-pink-100 px-2.5 py-1 text-xs font-black text-pink-700">FREE</span>
                </div>
                <h2 className="mt-3 text-lg font-black text-slate-900">免費資源中心</h2>
                <p className="mt-2 text-sm leading-relaxed text-slate-600">
                  免費圖片、提示詞與創作資源集中整理。
                </p>
              </Link>
          ...
          {SHOW_GROUP_BUY ? (
            <section className="mb-6 overflow-hidden rounded-3xl border border-orange-200 bg-gradient-to-r from-orange-50 via-white to-amber-50 p-5 shadow-sm order-1 sm:p-6">
            <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="inline-flex rounded-full bg-orange-600 px-3 py-1 text-xs font-black text-white">
                    亞尼克第一團・開放登記
                  </span>
                  <span className="text-sm font-bold text-orange-800">先登記，成團後才付款</span>
                </div>
                <h2 className="mt-3 text-2xl font-black text-slate-950">🧺 亞尼克生乳捲第一團</h2>
                <p className="mt-2 max-w-3xl text-sm leading-7 text-slate-700 sm:text-base">
                  團友可直接查看商品並登記數量，不需私訊粉絲團；達成團門檻後才由團媽通知付款。冷凍宅配未滿 10 條運費 200 元，滿 10 條免運。
                </p>
              </div>
              <div className="flex shrink-0 flex-col gap-2 sm:min-w-[190px]">
                <Link
                  to="/group-buy/yannick-first-group-buy"
                  className="inline-flex min-h-[50px] items-center justify-center rounded-2xl bg-orange-600 px-6 py-3 text-base font-black !text-white shadow-md transition hover:-translate-y-0.5 hover:bg-orange-700 hover:shadow-lg"
                  style={{ color: "#ffffff", WebkitTextFillColor: "#ffffff" }}
                >
                  查看商品並登記
                </Link>
                {loggedIn && <Link to="/my/group-buy-orders" className="text-center text-sm font-bold text-slate-600 underline">我的團購訂單</Link>}
                {isAdminEmail && <Link to="/admin/group-buy" className="text-center text-sm font-bold text-amber-800 underline">管理團購</Link>}
              </div>
            </div>
          </section>
          ...
          <section className="mb-8 overflow-hidden rounded-3xl border border-violet-200 bg-gradient-to-br from-violet-50 via-white to-orange-50 p-5 shadow-sm order-1 sm:p-7">
            <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
              <div>
                ...
                <h2 className="mt-4 text-2xl font-black leading-tight text-slate-950 sm:text-3xl">
                  商品圖、LINE 貼圖、名片與品牌視覺，都可以直接詢問
                </h2>
                <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-700 sm:text-base">
                  適合小店、個人品牌、業務與剛開始經營的創作者。先看實際作品與價格，再用網頁表單整理需求，不需要設定郵件程式。
                </p>
                <div className="mt-4 flex flex-wrap gap-2 text-sm font-bold text-slate-700">
                  {["商品圖升級", "LINE 貼圖客製", "名片設計＋代印", "品牌與禮盒圖稿"].map((item) => (
              ...
              <div className="flex flex-col gap-3 sm:flex-row lg:min-w-[220px] lg:flex-col">
                <Link
                  ...
                  className="inline-flex min-h-[52px] items-center justify-center rounded-2xl bg-gradient-to-r from-violet-600 to-fuchsia-500 px-6 py-3 text-base font-black !text-white shadow-md transition hover:-translate-y-0.5 hover:from-violet-700 hover:to-fuchsia-600 hover:shadow-lg"
                  style={{ color: "#ffffff", WebkitTextFillColor: "#ffffff" }}
                >
                  查看作品與接案價格
                </Link>
                <Link
                  to="/services/design-commission#inquiry"
                  className="inline-flex min-h-[50px] items-center justify-center rounded-2xl border border-orange-200 bg-white px-6 py-3 text-sm font-black text-orange-700 shadow-sm transition hover:-translate-y-0.5 hover:bg-orange-50"
                >
                  直接填寫詢問需求
                </Link>
              </div>
          ...
          <section className="mb-8 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm order-1">
            <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
              <div>
                <h2 className="text-xl font-black text-slate-900">所有工具快速入口</h2>
                <p className="mt-1 text-sm leading-relaxed text-slate-600">
                  依分類直接列出目前可用工具，先找入口、再往下看教學與文章。
                </p>
              </div>
                ...
                .map((tool) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="mt-2 text-sm leading-relaxed text-slate-600">
                        {tool.id === "images"
                          ? "部分圖片免費下載；完整版 1,583+ 張素材庫 NT$399，可一次取得。"
                          : t(tool.descKey)}
                      </p>
                    </Link>
                  );
          ...
          <section className="hidden">
            ...
            <div className="mb-6 p-6 bg-gradient-to-br from-emerald-50 to-cyan-50 rounded-xl border-2 border-emerald-200">
              ...
              <p className="text-sm leading-relaxed text-gray-700">
                目前保留部分免費圖片供下載；完整 1,583+ 張素材庫以 NT$399 提供。若想看到特定風格，也可以 Email 告訴 RxV。
              </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              >
                ...
                <p className="text-sm text-gray-700 mb-4 leading-relaxed">
                  部分圖片免費分享；完整版 1,583+ 張素材庫 NT$399，可一次取得。
                </p>
                <div
