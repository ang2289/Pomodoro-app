...
function trackStorefrontEvent(slug: string, type: "view" | "click", kind?: string) {
  if (!slug || typeof window === "undefined") return;
  const action = type === "view" ? "record-storefront-view" : "record-storefront-click";
  const payload = type === "view" ? { slug } : { slug, kind };
  fetch(`/api/main?action=${action}`, {
    method: "POST",
...
function contactTrackingKind(label: string, href: string) {
  const value = `${label} ${href}`.toLowerCase();
  if (value.includes("line") || value.includes("lin.ee") || value.includes("line.me")) return "line";
  if (value.includes("tel:" ) || label.includes("電話") || label.includes("撥打")) return "phone";
  if (value.includes("facebook")) return "facebook";
  if (value.includes("instagram")) return "instagram";
  if (value.includes("shopee") || label.includes("蝦皮")) return "shopee";
  return "website";
}
...
export default function PublicStorefrontPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        try {
          const viewKey = `rxv_storefront_viewed_${loadedStorefront.slug}`;
          if (typeof window !== "undefined" && !window.sessionStorage.getItem(viewKey)) {
            window.sessionStorage.setItem(viewKey, "1");
            trackStorefrontEvent(loadedStorefront.slug, "view");
          }
        } catch {
          trackStorefrontEvent(loadedStorefront.slug, "view");
        }
  ...
  const trackClick = (kind: string) => {
    trackStorefrontEvent(storefront.slug, "click", kind);
  };
            ...
            <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
              ...
              <div className="flex flex-wrap gap-3">
                {visiblePrimaryCta ? (
                  <a
                    href={primaryCtaHref}
                    onClick={() => trackClick("primary_cta")}
              ...
              <div className="mt-6 grid w-full max-w-[560px] grid-cols-1 gap-3 sm:grid-cols-2">
                {contactLinks.map((link) => (
                  <a
                    key={link.label}
                    href={link.href}
                    onClick={() => trackClick(contactTrackingKind(link.label, link.href))}
                  ...
                  <div className="p-5">
                    ...
                    {item.button_url ? (
                      <a
                        ...
                        onClick={() => trackClick(contactTrackingKind(getActionLabel(item.button_label, item.button_url), safeHref(item.button_url)))}
                        target="_blank"
                        rel="noreferrer"
              ...
              {contactDetails.map((detail) => (
                ...
                >
                  ...
                  {detail.href ? (
                    <a
                      href={detail.href}
                      onClick={() => trackClick(contactTrackingKind(detail.label, detail.href || ""))}
