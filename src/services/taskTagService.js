import { DEFAULT_TAG_COLORS } from '../types/TaskTag';
...
// 初始化預設標籤
export const initializeDefaultTaskTags = () => {
...
// 獲取標籤（包含使用次數）
export const getTaskTagsWithCount = () => {
...
// 新增標籤
export const addTaskTag = (name, color, createdBy = 'user') => {
    const newTag = {
        id: generateUUID(),
        name: name.trim(),
        color,
...
// 更新標籤
export const updateTaskTag = (id, name, color) => {
    const tags = getTaskTags();
    const tagIndex = tags.findIndex(tag => tag.id === id);
    if (tagIndex === -1)
        return false;
    // 不允許編輯預設標籤
    if (tags[tagIndex].isDefault)
        return false;
