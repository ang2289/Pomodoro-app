import { GroupMember, JoinGroupData } from '../types/GroupMember'
...
export const saveGroupMember = (member: GroupMember): void => {
...
export const getGroupMembers = (): GroupMember[] => {
...
export const getGroupsByUserId = (userId: string): Group[] => {
  ...
  return allGroups.filter((group: Group) => userGroups.includes(group.id))
}

// 檢查使用者是否已加入群組
export const isUserInGroup = (userId: string, groupId: string): boolean => {
  const members = getGroupMembers()
  return members.some(member => member.userId === userId && member.groupId === groupId)
}

// 加入群組
export const joinGroup = (data: JoinGroupData): { success: boolean; message: string; group?: Group } => {
  try {
    // 尋找群組
...
// 離開群組
export const leaveGroup = (userId: string, groupId: string): boolean => {
  try {
    const members = getGroupMembers()
