...
export default defineConfig(({ mode }) => {
  ...
  plugins: [
    {
      name: "rxv-ignore-sqlite-hmr",
      handleHotUpdate(ctx: any) {
        const file = String(ctx?.file || "").replace(/\\/g, "/");
        if (/\/data\//i.test(file) || /\.db(?:-(?:wal|shm))?$/i.test(file)) return [];
        return undefined;
      },
    },
    react(),
    {
      name: 'rxv-local-image-admin-api-v4',
  ...
  server: {
    // V40.3.8: SQLite WAL/SHM change constantly while the publisher runs.
    // Ignore them so Vite does not full-reload the page and erase an active CSV import.
    watch: {
      ignored: [
        "**/data/**",
        "**/*.db",
