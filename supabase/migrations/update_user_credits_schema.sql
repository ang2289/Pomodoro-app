...
-- ==========================================
-- 1. 檢查並建立/更新 user_credits 表
-- ==========================================

-- 如果表不存在，建立新表（符合用戶需求的結構）
CREATE TABLE IF NOT EXISTS public.user_credits (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID UNIQUE NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  total_credits INTEGER NOT NULL DEFAULT 10000,
  used_credits INTEGER NOT NULL DEFAULT 0,
  remaining_chars INTEGER GENERATED ALWAYS AS (total_credits - used_credits) STORED, -- 計算欄位，向後兼容
...
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE public.user_credits ADD COLUMN created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW() NOT NULL;
...
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE public.user_credits ADD COLUMN updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW() NOT NULL;
  END IF;
END $$;

-- 確保 remaining_chars 欄位存在（向後兼容）
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
...
-- ==========================================
-- 5. 建立/更新觸發器：自動同步 remaining_chars
-- ==========================================

-- 建立觸發器函數：當 total_credits 或 used_credits 更新時，自動計算 remaining_chars
