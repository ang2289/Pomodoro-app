...
async function imageBundleAdminRequest<T>(action: string, method: 'GET' | 'POST', body?: unknown): Promise<T> {
  ...
  const response = await fetch(`/api/image-bundle-orders?action=${encodeURIComponent(action)}`, {
    method,
    headers: {
                ...
                <article key={order.id} className="rounded-2xl border border-amber-200 p-5 shadow-sm">
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div className="min-w-0">
                      ...
                      <div className="mt-3 grid gap-2 text-sm text-slate-700 sm:grid-cols-2">
                        <p><span className="font-bold">商品：</span>1,584+ 高畫質圖片素材庫完整版</p>
                        ...
                        <p><span className="font-bold">匯款日期：</span>{order.transfer_date || '—'}</p>
                        ...
                        {order.status === 'approved' ? <p><span className="font-bold">下載連結期限：</span>{formatDate(order.download_expires_at)}　<span className="text-slate-500">R2 臨時連結暫不計次</span></p> : null}
