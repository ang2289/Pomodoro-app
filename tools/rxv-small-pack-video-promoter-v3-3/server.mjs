...
const TOOL_DIR = path.resolve(ROOT, 'tools', 'rxv-small-pack-video-promoter-v3-3');
const PROJECT_DRIVE_ROOT = path.parse(ROOT).root || ROOT;
const TEMP_ROOT = path.join(os.tmpdir(), 'rxv-small-pack-video-promoter-v3-3');
...
app.post('/api/render', upload.fields([
  ...
  try {
    ...
    for (let i = 0; i < imageFiles.length; i += 1) {
      ...
      await makeVerticalFrame(imageFiles[i].path, framePath, copy.top, copy.bottom);
    ...
    if (musicFile) {
      ...
      args.push(
        ...
        '-c:a', 'aac', '-b:a', '192k',
        '-t', String(totalSeconds), '-movflags', '+faststart',
        outputPath,
      );
    } else {
      args.push(
    ...
    console.error('[RXV promo v3.3 render]', error);
...
app.use((error, _req, res, _next) => {
  console.error('[RXV promo v3.3]', error);
...
app.listen(PORT, HOST, async () => {
  ...
  console.log('');
  console.log('RXV 小包短影音推廣器 v3.3｜MP3 背景音樂版');
  console.log(`網址：${url}`);
