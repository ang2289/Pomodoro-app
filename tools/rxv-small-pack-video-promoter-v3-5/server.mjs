...
const TOOL_DIR = path.resolve(ROOT, 'tools', 'rxv-small-pack-video-promoter-v3-5');
const PROJECT_DRIVE_ROOT = path.parse(ROOT).root || ROOT;
const TEMP_ROOT = path.join(os.tmpdir(), 'rxv-small-pack-video-promoter-v3-5');
...
async function makeCtaFrame(backgroundPath, outputPath, product, ctaUrl, lineId) {
...
app.post('/api/render', upload.fields([
  ...
  try {
    ...
    console.error('[RXV promo v3.5 render]', error);
...
app.use((error, _req, res, _next) => {
  console.error('[RXV promo v3.5]', error);
