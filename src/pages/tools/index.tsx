  ...
  const renderHomeQuickButton = (card: (typeof toolCards)[number]) => {
    ...
    return (
      <Link
        ...
        className="inline-flex min-h-[46px] items-center justify-center rounded-full border border-blue-200 bg-white px-4 py-2.5 text-sm font-bold text-slate-900 shadow-sm transition hover:-translate-y-0.5 hover:bg-blue-50 hover:text-blue-700 hover:shadow-md"
      >
        <span className="mr-1.5" aria-hidden>{card.icon}</span>
        {t(card.titleKey)}
      </Link>
    );
  };
        ...
        <div className="mx-auto w-full max-w-6xl px-4 pt-5 sm:px-6 lg:px-8">
          <div className="overflow-hidden rounded-[1.5rem] border border-emerald-100 bg-white shadow-sm">
            ...
            <div className="border-t border-emerald-100 bg-gradient-to-r from-cyan-50 via-white to-emerald-50 p-5 sm:p-6">
              ...
              <div className={`mt-5 grid w-full gap-2 ${loggedIn ? 'sm:grid-cols-4' : 'sm:grid-cols-3'}`}>
                <Link
                  to="/tools/product-showcase-page"
                  className="inline-flex min-h-[48px] items-center justify-center rounded-xl bg-gradient-to-r from-cyan-600 to-sky-500 px-5 py-3 text-sm font-black !text-white shadow-md transition hover:-translate-y-0.5 hover:from-cyan-700 hover:to-sky-600 hover:shadow-lg"
                  style={{ color: "#ffffff", WebkitTextFillColor: "#ffffff" }}
                >
                  商品展示頁 NT$199／3 個月
                </Link>
                <Link
                  to="/pricing"
                  className="inline-flex min-h-[48px] items-center justify-center rounded-xl border border-emerald-200 bg-white px-5 py-3 text-sm font-black text-emerald-700 shadow-sm transition hover:-translate-y-0.5 hover:border-emerald-300 hover:bg-emerald-50 hover:shadow-md"
                >
                  查看商品圖點數
                </Link>
                <Link
                ...
                {loggedIn ? (
                  <button
                    type="button"
                    onClick={() => openStorefrontSettings("product-image")}
                    className="inline-flex min-h-[48px] items-center justify-center rounded-xl border border-sky-200 bg-white px-5 py-3 text-sm font-black text-sky-700 shadow-sm transition hover:-translate-y-0.5 hover:border-sky-300 hover:bg-sky-50 hover:shadow-md"
                  >
                    設定我的商品展示頁
                  </button>
                ) : null}
          ...
          <section className="mb-8 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm order-1">
            ...
            <div className="space-y-5">
              {homeToolGroups.map((group) => (
                <div key={group.key}>
                  <h3 className="mb-2 text-sm font-black text-slate-700">{group.title}</h3>
