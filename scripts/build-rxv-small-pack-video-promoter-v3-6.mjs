...
async function makeCtaFrame(backgroundPath, outputPath, product, ctaUrl, lineId, qrOptions = {}) {
  ...
  const finalQrUrl = normalizeQrUrl(qrUrl || ctaUrl || 'pomodoro-app-eight-rouge.vercel.app/image-packs');
...
const qrUi = `  <div class="grid g4" style="margin-top:14px">
    ...
    <div><label class="label">QR Code 網址</label><input id="qrUrl" value="https://pomodoro-app-eight-rouge.vercel.app/image-packs" /><div class="muted">可改成商品頁、LINE 加好友或其他網址。</div></div>
...
html = html.replace(infoNeedle, `<br>成交 CTA：${data.enableCta?'最後 2 秒':'關閉'}<br>QR Code：${data.enableQr?'開啟':'關閉'}<br>QR 網址：${esc(data.qrUrl||'')}<br>QR 位置：${esc(data.qrPosition||'')}<br>LINE：${esc(data.lineId||'')}<br>輸出：${esc(data.outputDir)}`);
