...
export const saveTaskRegistration = (registration) => {
...
export const getTaskRegistrations = () => {
    const registrations = localStorage.getItem('pomodoro-task-registrations');
    return registrations ? JSON.parse(registrations) : [];
};
// 根據任務 ID 取得報名列表
export const getTaskRegistrationsByTaskId = (taskId) => {
    const registrations = getTaskRegistrations();
    return registrations.filter(registration => registration.taskId === taskId);
};
// 檢查使用者是否已報名任務
export const isUserRegisteredForTask = (userId, taskId) => {
    const registrations = getTaskRegistrations();
    return registrations.some(registration => registration.userId === userId && registration.taskId === taskId);
};
// 取得使用者在特定任務的報名資料
export const getUserTaskRegistration = (userId, taskId) => {
    const registrations = getTaskRegistrations();
    return registrations.find(registration => registration.userId === userId && registration.taskId === taskId) || null;
};
// 報名任務
export const registerForTask = (data) => {
    const registration = {
        id: generateUUID(),
        taskId: data.taskId,
        userId: data.userId,
...
// 更新領貨狀態
export const updateTaskRegistrationStatus = (registrationId, isCompleted) => {
    try {
        const registrations = getTaskRegistrations();
...
// 取消報名
export const cancelTaskRegistration = (registrationId) => {
    try {
        const registrations = getTaskRegistrations();
