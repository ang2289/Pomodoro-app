...
function dashboardHtml() {
  return [
    ...
    "async function refreshAll(){try{setMsg('同步中…');const s=await api('/api/status');document.getElementById('total').textContent=s.total;document.getElementById('draft').textContent=s.draft;document.getElementById('published').textContent=s.published;document.getElementById('remaining').textContent=s.remaining;const d=await api('/api/drafts');renderDrafts(d.items||[]);const h=await api('/api/published?limit=20');renderHistory(h.items||[]);setMsg('已同步最新資料｜來源：'+s.manifestSource)}catch(e){setMsg('錯誤：'+e.message)}}",
    ...
    "function renderDrafts(items){const box=document.getElementById('drafts');if(!items.length){box.innerHTML='<div class=\"muted\">目前沒有待發布內容。按上方按鈕建立。</div>';return}box.innerHTML=items.map(x=>\`<div class='card'><div><img src='\${esc(x.image_url)}' alt=''><div class='small' style='margin-top:6px'>\${esc(x.image_id)}</div></div><div><span class='tag'>\${esc(x.category||'其他')}</span><h3>\${esc(x.title||'圖片素材')}</h3><textarea id='cap_\${esc(x.image_id)}'>\${esc(x.caption||'')}</textarea><div class='actions'><button class='btn green' data-action='copy' data-id='\${encodeURIComponent(x.image_id)}'>複製文案</button><a class='btn gray' style='text-decoration:none' target='_blank' href='/api/image?imageId=\${encodeURIComponent(x.image_id)}'>開啟圖片</a><button class='btn' data-action='published' data-id='\${encodeURIComponent(x.image_id)}'>標記已發布</button><button class='btn red' data-action='release' data-id='\${encodeURIComponent(x.image_id)}'>取消待發</button></div></div></div>\`).join('');box.querySelectorAll('[data-action]').forEach(btn=>btn.addEventListener('click',()=>{const id=decodeURIComponent(btn.dataset.id||'');const action=btn.dataset.action;if(action==='copy')void copyCap(id);if(action==='published')void markPublished(id);if(action==='release')void releaseDraft(id)}))}",
