// 解析領貨時間文字，提取日期和時間
export const parseDeliveryTime = (deliveryTime: string): Date | null => {
  try {
    // 嘗試解析各種時間格式
    const now = new Date()
...
// 計算距離目標時間的剩餘時間
export const getTimeRemaining = (targetTime: Date): {
  totalHours: number
  days: number
  hours: number
  minutes: number
...
// 檢查是否應該顯示提醒
export const shouldShowReminder = (timeRemaining: ReturnType<typeof getTimeRemaining>): boolean => {
