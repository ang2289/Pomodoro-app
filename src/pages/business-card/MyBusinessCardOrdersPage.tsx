...
function getBrandWebsiteGiftMonths(order: { serviceType: string; itemAmountNtd: number }) {
  return order.serviceType === 'layout' && Number(order.itemAmountNtd || 0) >= 349 ? 3 : 0
}
...
export default function MyBusinessCardOrdersPage() {
  ...
  useEffect(() => {
    if (createdFromOrderPage && !createdNoticeShown.current) {
      ...
      setNotice('名片訂單已建立。請先依下方金額完成銀行匯款並回填資料；工作室核對入帳後才會開始人工排版。符合人工排版贈送資格的訂單，會同步建立一頁式品牌網站草稿。')
    }
  ...
  const brandWebsiteGiftMonths = detail ? getBrandWebsiteGiftMonths(detail.order) : 0
          ...
          <div className="grid gap-6 xl:grid-cols-[310px_minmax(0,1fr)]">
            <aside className="space-y-3 xl:sticky xl:top-5 xl:h-fit">
              ...
              {orders.map((order) => (
                <button key={order.id} type="button" onClick={() => setSelectedId(order.id)} className={`w-full rounded-2xl border p-4 text-left shadow-sm transition ${selectedId === order.id ? 'border-cyan-500 bg-cyan-50 ring-2 ring-cyan-100' : 'border-slate-200 bg-white hover:border-cyan-300'}`}>
                  ...
                  <p className="mt-1 text-xs text-slate-500">{formatDate(order.createdAt)}・NT${order.totalAmountNtd.toLocaleString()}</p>{getBrandWebsiteGiftMonths(order) ? <span className="mt-2 inline-flex rounded-full bg-violet-100 px-2.5 py-1 text-[11px] font-black text-violet-800">送品牌網站 3 個月</span> : null}
                </button>
              ))}
            </aside>
            ...
            <section className="min-w-0 rounded-3xl bg-white p-5 shadow-sm sm:p-6">
              {detailLoading || !detail || !selectedOrder ? <p className="p-8 text-center text-slate-600">正在讀取訂單內容…</p> : <>
                ...
                <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-4"><div className="rounded-2xl bg-slate-50 p-4"><p className="text-xs font-bold text-slate-500">印刷規格</p><p className="mt-1 font-black text-slate-950">{sideLabel(detail.order.printSide)}／{finishLabel(detail.order.finishType)}</p><p className="mt-1 text-sm text-slate-600">{detail.order.boxCount} 盒（{detail.order.quantityCards.toLocaleString()} 張）</p></div><div className="rounded-2xl bg-slate-50 p-4"><p className="text-xs font-bold text-slate-500">訂單金額</p><p className="mt-1 text-xl font-black text-emerald-700">NT${detail.order.totalAmountNtd.toLocaleString()}</p><p className="mt-1 text-sm text-slate-600">含宅配：{detail.order.shippingFeeNtd ? `NT$${detail.order.shippingFeeNtd}` : '免運'}</p></div><div className="rounded-2xl bg-slate-50 p-4">{paymentRequired ? <><p className="text-xs font-bold text-amber-700">下一步</p><p className="mt-1 font-black text-amber-900">請先完成銀行匯款</p><p className="mt-1 text-sm text-slate-600">核對入帳後才會開始人工排版。</p></> : <><p className="text-xs font-bold text-slate-500">文字修改</p><p className="mt-1 font-black text-slate-950">已使用 {detail.order.revisionCount}／1 次</p><p className="mt-1 text-sm text-slate-600">預覽稿完成後可提出一次修改。</p></>}</div>{brandWebsiteGiftMonths ? <div className="rounded-2xl border border-violet-200 bg-violet-50 p-4"><p className="text-xs font-bold text-violet-700">免費贈送</p><p className="mt-1 font-black text-violet-950">一頁式品牌網站基本版</p><p className="mt-1 text-sm text-violet-800">付款確認後自動建立草稿，可使用 {brandWebsiteGiftMonths} 個月。</p></div> : null}</div>
                ...
                {detail.order.status === 'payment_reported' ? <section className="mt-6 rounded-2xl border border-amber-200 bg-amber-50 p-5"><h3 className="font-black text-amber-950">已收到匯款回報</h3><p className="mt-2 text-sm leading-relaxed text-amber-900">工作室會依實際銀行入帳核對；確認後訂單會顯示「已確認入帳」，接著開始人工排版。{brandWebsiteGiftMonths ? ' 此訂單符合贈送資格，確認入帳後會同步建立一頁式品牌網站草稿。' : ''}</p></section> : null}
                {detail.order.status === 'payment_verified' ? <section className="mt-6 rounded-2xl border border-cyan-200 bg-cyan-50 p-5"><h3 className="font-black text-cyan-950">匯款已確認，開始人工排版</h3><p className="mt-2 text-sm leading-relaxed text-cyan-900">工作室已核對入帳，正在依你選擇的模板與資料進行排版；完成後會在此顯示正、背面預覽稿。{brandWebsiteGiftMonths ? ' 一頁式品牌網站基本版草稿也會同步建立。' : ''}</p></section> : null}
                ...
                {detail.digitalProfile ? <section className="mt-6 rounded-2xl border border-violet-200 bg-violet-50 p-5"><h3 className="font-black text-violet-950">🎁 一頁式品牌網站基本版已建立</h3><p className="mt-2 text-sm leading-relaxed text-violet-900">{detail.digitalProfile.status === 'published' ? '網站已公開，可直接分享給客戶。' : '網站草稿已建立；請登入後補齊內容並決定是否公開。'}</p><div className="mt-4 flex flex-wrap gap-3"><Link to="/settings/storefront" className="inline-flex rounded-xl bg-violet-600 px-4 py-2.5 font-black !text-white hover:bg-violet-700" style={{ color: '#fff', WebkitTextFillColor: '#fff' }}>前往網站設定</Link>{detail.digitalProfile.status === 'published' ? <a href={detail.digitalProfile.publicPath} target="_blank" rel="noreferrer" className="inline-flex rounded-xl border border-violet-300 bg-white px-4 py-2.5 font-black text-violet-800 hover:bg-violet-100">開啟公開網站</a> : null}</div><p className="mt-3 text-xs text-violet-800">使用期限至：{formatDate(detail.digitalProfile.expiresAt)}</p></section> : brandWebsiteGiftMonths ? <section className="mt-6 rounded-2xl border border-violet-200 bg-violet-50 p-5"><h3 className="font-black text-violet-950">🎁 一頁式品牌網站贈送資格</h3><p className="mt-2 text-sm leading-relaxed text-violet-900">本筆人工排版名片符合免費贈送資格。付款確認後，系統會建立 3 個月的一頁式品牌網站基本版草稿。</p></section> : null}
